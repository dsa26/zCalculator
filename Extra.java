// EXTRA STUFF I TRIED WITH TIME ZONES -- DIDN'T GET AROUND TO MAKING IT WORK
...
// // Used AI to figure out the proper syntax for the declaring an array
// consisting
// // of custom objects

// private static final HashMap<TimeZones, RelativeDate[]> tzOffsets = new
// HashMap<TimeZones, RelativeDate[]>() {
// {
// put(TimeZones.AmericaLos_Angeles, new RelativeDate[] { new RelativeDate(0,
// -8), new RelativeDate(0, -7) });
// put(TimeZones.AmericaChicago, new RelativeDate[] { new RelativeDate(0, -6),
// new RelativeDate(0, -5) });
// put(TimeZones.AmericaNew_York, new RelativeDate[] { new RelativeDate(0, -5),
// new RelativeDate(0, -4) });
// put(TimeZones.EuropeLondon, new RelativeDate[] { new RelativeDate(0, 0), new
// RelativeDate(0, 1) });
// put(TimeZones.AfricaAccra, new RelativeDate[] { new RelativeDate(0, 0), new
// RelativeDate(0, 0) });
// put(TimeZones.EuropeBerlin, new RelativeDate[] { new RelativeDate(0, 1), new
// RelativeDate(0, 2) });
// put(TimeZones.EuropeAthens, new RelativeDate[] { new RelativeDate(0, 2), new
// RelativeDate(0, 3) });
// put(TimeZones.EuropeKyiv, new RelativeDate[] { new RelativeDate(0, 2), new
// RelativeDate(0, 3) });
// put(TimeZones.AsiaAmman, new RelativeDate[] { new RelativeDate(0, 3), new
// RelativeDate(0, 3) });
// put(TimeZones.AsiaDubai, new RelativeDate[] { new RelativeDate(0, 4), new
// RelativeDate(0, 4) });
// put(TimeZones.AsiaKolkata, new RelativeDate[] { new RelativeDate(30, 5), new
// RelativeDate(30, 5) });
// put(TimeZones.AsiaShanghai, new RelativeDate[] { new RelativeDate(0, 8), new
// RelativeDate(0, 8) });
// put(TimeZones.AsiaSeoul, new RelativeDate[] { new RelativeDate(0, 9), new
// RelativeDate(0, 9) });
// put(TimeZones.AustraliaSydney, new RelativeDate[] { new RelativeDate(0, 10),
// new RelativeDate(0, 11) });
// }
// };

// // private static final HashMap<TimeZones, Date[]> tzDST =
// newHashMap<TimeZones,
// // Date[]>()
// // {
// // {
...
// return diff.Minutes * 60 + diff.Hours * 3600 + (diff.Days - 1) * 86400 +
// diff.Months * 2592000
// + diff.Years * 31536000;
// }
